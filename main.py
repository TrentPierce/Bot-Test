...
def get_channel():
    ...
    if len(sys.argv) >= 2:
        ...
        print(
            "An error has occurred while trying to read arguments. Did you specify the channel?"
        )
        sys.exit(1)
...
def get_proxies():
    ...
    try:
        ...
        print(
            "An error has occurred while trying to read the list of proxies: %s"
            % e.strerror)
        sys.exit(1)
        ...
        try:
            if time.time() - proxy_data['time'] >= random.randint(1, 5):
                ...
                with requests.Session() as s:
                    response = s.head(current_url,
                                      proxies=current_proxy,
                                      headers=headers)
                print(
                    f"Sent HEAD request with {current_proxy['http']} | {response.status_code} | {response.request} | {response}"
                )
...
if __name__ == "__main__":
    ...
    if len(sys.argv) == 1:
        channel_url = "https://www.twitch.tv/Stemo13"
    ...
    while True:
        try:
            for i in range(0, max_nb_of_threads):
                threaded = Thread(target=open_url,
                                  args=(all_proxies[random.randint(
                                      0, len(all_proxies))], ))
